 ...
 */
public class WeightRandom<K extends Number, V> {
    private TreeMap<Double, V> weightMap = new TreeMap<Double, V>();
    private static final Logger logger = LoggerFactory.getLogger(WeightRandom.class);

    public WeightRandom(List<Pair<V, K>> list) {
        Preconditions.checkNotNull(list, "list can NOT be null!");
        for (Pair<V, K> pair : list) {
